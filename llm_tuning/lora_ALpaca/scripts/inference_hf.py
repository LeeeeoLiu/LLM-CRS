    ...
    with torch.no_grad():
        if args.interactive:
            ...
            for index, example in tqdm(enumerate(examples)):
                # if index < 49671 :continue
                if args.with_prompt is True:
                    input_text = generate_prompt(instruction=example['instruction'],input=example['input'])
                else:
                    input_text = example
                inputs = tokenizer(input_text,return_tensors="pt")  #add_special_tokens=False ?
                generation_output = model.generate(
                ...
                results.append({"label":example['output'],"predict":response})
